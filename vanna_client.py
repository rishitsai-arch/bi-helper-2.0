...
class MyVanna(ChromaDB_VectorStore, OpenAI_Chat):
    ...
    def __init__(self, config=None):
        ...
        # This version of vanna doesn't reliably honor base_url from config,
        # so we override the underlying client directly to force it to Groq's
        # endpoint instead of OpenAI's default.
        self.client = OpenAI(
...
def ask_vanna(vn, question: str):
    ...
    database, and returns the SQL, the resulting DataFrame, and (if
    applicable) a Plotly figure visualizing the result.
    """
    ...
    fig = None
    try:
        # Vanna only bothers generating a chart if the shape of the data
        # makes sense to plot (e.g. skips single-value or purely text results)
        if vn.should_generate_chart(question=question, sql=sql, df=df):
            plotly_code = vn.generate_plotly_code(question=question, sql=sql, df=df)
            fig = vn.get_plotly_figure(plotly_code=plotly_code, df=df)
    except Exception as e:
        # Non-fatal — table results still come through even if charting fails
